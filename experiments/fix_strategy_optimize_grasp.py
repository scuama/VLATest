...
# 搜索参数
DEFAULT_TOTAL_ATTEMPTS = 10  # 总搜索次数（粗搜索+精细搜索）
...
DIRECTION_OFFSETS = {
    "left": {"x": (-0.02, -0.005), "y": (-0.003, 0.003)},    # 左：x负向
    "right": {"x": (0.005, 0.02), "y": (-0.003, 0.003)},     # 右：x正向
    "up": {"x": (-0.003, 0.003), "y": (-0.02, -0.005)},      # 上：y负向
    "down": {"x": (-0.003, 0.003), "y": (0.005, 0.02)},      # 下：y正向
    "left-up": {"x": (-0.02, -0.005), "y": (-0.02, -0.005)}, # 左上
    "left-down": {"x": (-0.02, -0.005), "y": (0.005, 0.02)}, # 左下
    "right-up": {"x": (0.005, 0.02), "y": (-0.02, -0.005)},  # 右上
    "right-down": {"x": (0.005, 0.02), "y": (0.005, 0.02)},  # 右下
}
...
示例用法:
  ...
  python3 auto_find_successful_grasp.py 13 left-up --base_dir results/t-grasp_n-100/openvla-7b_2024
    ...
    # 构建episode目录
    episode_dir = os.path.join(args.base_dir, args.episode_id)
    ...
    print(f"🎯 任务: {args.task} ({task_name})")
    ...
    if coarse_attempts > 0:
        ...
        for attempt in range(1, coarse_attempts + 1):
            result = run_single_attempt(
                ...
                episode_dir, task_name, args.task,
    ...
    print(f"📦 原始备份: {os.path.join(episode_dir, 'options.json.original_backup')}")
    print("=" * 70)
